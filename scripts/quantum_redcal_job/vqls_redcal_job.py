...
from hera_cal import quantum_redcal as om 
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--quantum_solver", default='vqls', help="type of solver", type=str)
    # parser.add_argument("--num_reps", default=3, help="Number repetition of the ansatz", type=int)
    # parser.add_argument("--num_shots", default=0, help="Number of shots", type=int)
    # parser.add_argument("--matrix_decomposition", default='symmetric', help="method to decompose the matrix", type=str)
    ...
    # define the ansatz
    num_qubits = int(np.ceil(np.log(len(antpos))))
    ansatz = RealAmplitudes(num_qubits, entanglement='full', reps=3, insert_barriers=False)

    # define the optimizer
    opt1 = opt.COBYLA(maxiter=5)
    opt2 = opt.CG(maxiter=20, gtol=1E-8, tol=1E-8)
    optimizer = [opt1] #, opt2]
    ...
    # define the solver
    solver = VQLS(estimator, ansatz, optimizer, sampler=sampler, 
                        options={'matrix_decomposition':'symmetric',
                                 'shots':None, 'reuse_matrix':False},
                        gradient=FiniteDiffEstimatorGradient(Estimator(),1E-5))

    # calibrate
    rc = om.QuantumRedundantCalibrator(reds, solver=solver)
