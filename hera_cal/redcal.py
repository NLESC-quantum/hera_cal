...
def combine_reds(reds1, reds2, unfiltered_reds=None):
    ...
    combined_reds = [
        sorted(red, key=lambda x: x[0])
        for red in sorted(combined_reds, key=len, reverse=True)
    ]
...
def remove_degen_gains(reds, gains, degen_gains=None, mode="phase", pol_mode="1pol"):
    ...
    assert pol_mode in [
        ...
        "4pol",
        "4pol_minV",
    ], f"Unrecognized pol_mode: {pol_mode}"
    assert mode in ("phase", "complex"), "Unrecognized mode: %s" % mode
    ants = list(
        set(ant for gp in reds for bl in gp for ant in split_bl(bl) if ant in gains)
    )
    gainPols = np.array(
        [ant[1] for ant in ants]
    ...
    if pol_mode == "2pol":
        ...
        new_gains = remove_degen_gains(
            reds0, pol0_gains, degen_gains=degen_gains, mode=mode, pol_mode="1pol"
        )
        new_gains.update(
            remove_degen_gains(
                reds1, pol1_gains, degen_gains=degen_gains, mode=mode, pol_mode="1pol"
            )
        )
        return new_gains
    ...
    if mode == "phase":
        ...
        for pol in antpols:
            ...
            gainSols[gainPols == pol] *= (degenMeanSqAmplitude / meanSqAmplitude) ** 0.5
...
class RedSol:
    ...
    def __init__(self, reds, gains={}, vis={}, sol_dict={}):
        ...
        if len(sol_dict) > 0:
            if (len(gains) > 0) or (len(vis) > 0):
                raise ValueError(
                    "If sol_dict is not empty, both gains and vis must be."
                )
    ...
    def __getitem__(self, key):
        ...
        if len(key) == 3:  # visibility key
            ...
            raise KeyError(
                "RedSol keys should be length-2 (for gains) or length-3 (for visibilities)."
            )
    ...
    def update_vis_from_data(self, data, wgts=None, reds_to_update=None):
        ...
        for grp in reds_to_update:
            ...
            if np.any(np.sum(wgts_here, axis=0) == 0):
                ...
                not_totally_flagged_bls = [
                    bl for bl, wgt in zip(grp, wgts_here) if not np.all(wgt == 0)
                ]
                flag_waterfall = np.all(
                    [wgts[bl] == 0 for bl in not_totally_flagged_bls], axis=0
                )
                wgts_here = [
                    np.where(flag_waterfall, 1, wgts[bl])
                    if bl in not_totally_flagged_bls
                    else wgts[bl]
                    for bl in grp
                ]

            self.vis[grp[0]] = np.average(
                [self.calibrate_bl(bl, data[bl]) for bl in grp],
                axis=0,
                weights=wgts_here,
            )
    ...
    def chisq(self, data, data_wgts, gain_flags=None):
        ...
        chisq, _, chisq_per_ant, _ = utils.chisq(
            ...
            reds=self.reds,
            split_by_antpol=split_by_antpol,
        )
    ...
    def normalized_chisq(self, data, data_wgts):
        ...
        """
        chisq, chisq_per_ant = normalized_chisq(
            data, data_wgts, self.reds, self.vis, self.gains
        )
        return chisq, chisq_per_ant
...
def _firstcal_align_bls(bls, freqs, data, norm=True, wrap_pnt=(np.pi / 2)):
    ...
    def process_pair(gp1, gp2):
        ...
        bin_shifts = (
            (delta1 + delta2) / 2
            + utils.quinn_tau(delta1**2)
            - utils.quinn_tau(delta2**2)
        )
        ...
        _data[gp1 + gp2] = _data[gp1] + _data[gp2] * phasor * np.exp(
            np.complex64(1j) * off
        )
        return gp1 + gp2
    ...
    dly_offs = {
        k: (v[0], _wrap_phs(v[1], wrap_pnt=wrap_pnt)) for k, v in dly_offs.items()
    }
    return dly_offs
...
class RedundantCalibrator:
    ...
    def _set_reds(self, reds):
        ...
        self._ants_in_reds = set(
            [ant for red in self.reds for bl in red for ant in split_bl(bl)]
        )
    ...
    def _firstcal_iteration(
        ...
        for bls in self.reds:
            ...
            if fc_min_vis_per_ant is not None:
                ...
                if np.all(
                    np.array(list(ants_used_count.values())) >= fc_min_vis_per_ant
                ):
    ...
    def lincal(
        ...
        ls = self._solver(
            linsolve.LinProductSolver, data, sol0=sol0pack, wgts=wgts, sparse=sparse
        )
...
def predict_chisq_per_bl(reds, pol_separable=None):
    ...
    if pol_separable:
        ...
        for pol, polreds in reds_by_pol.items():
            predicted_chisq_per_bl.update(
                predict_chisq_per_bl(polreds, pol_separable=False)
            )
        ...
        eqinds = [split_bl(bl) + (u,) for u, gp in enumerate(reds) for bl in gp]
        # eqinds = (ant-prm-index, ant-prm-index, red-gp-prm-index) for each bl
        eqinds = [(ants[ai], ants[aj], len(ants) + ug) for ai, aj, ug in eqinds]
        # eqinds = (AtA x/y indices = outer product of prm indices) for each bl
        eqinds = [
            (
                np.array([ai, ai, ai, aj, aj, aj, ug, ug, ug]),
        ...
        for ci, cj, cu in ((1, 1, 1), (1, -1, 1)):
            ...
            diag_sum = np.array([np.sum(coeffs * AtAi[x, y]) for x, y in eqinds])
...
def linear_cal_update(
    ...
    for bl in bls:
        ...
        dt = infer_dt(
            data.times_by_bl, bl, default_dt=SEC_PER_DAY**-1
        )  # pick reasonable default for equal weights
...
def expand_omni_sol(cal, all_reds, data, nsamples):
    ...
    dts_by_bl = DataContainer(
        {
            bl: infer_dt(data.times_by_bl, bl, default_dt=SEC_PER_DAY**-1)
            * SEC_PER_DAY
...
def redundantly_calibrate(
    ...
    if run_omnical or compute_chisq:
        dts_by_bl = DataContainer(
            {
                bl: infer_dt(times_by_bl, bl, default_dt=SEC_PER_DAY**-1)
                * SEC_PER_DAY
                for bl in red_bls
            }
        )
...
def expand_omni_vis(
    sol, expanded_reds, data, nsamples=None, chisq=None, chisq_per_ant=None
):
    ...
    reds_to_solve = [
        red for red in good_ants_reds if not np.any([bl in sol.vis for bl in red])
    ]
    data_bls_to_use = [
        bl
        for red in reds_to_solve
        for bl in red
    ...
    if len(reds_to_solve) > 0:
        ...
        if (chisq is not None) or (chisq_per_ant is not None):
            chisq, chisq_per_ant = normalized_chisq(
                data, data_wgts, good_ants_reds, sol.vis, sol.gains
            )
...
def expand_omni_gains(sol, expanded_reds, data, nsamples=None, chisq_per_ant=None):
    ...
    while True:
        bls_to_use = set(
            [
                bl
                for red in expanded_reds
                for bl in red
                if (
                    np.any([bl in sol.vis for bl in red])
                    and (
                        (split_bl(bl)[0] not in sol.gains)
                        ^ (split_bl(bl)[1] not in sol.gains)
                    )
                )
            ]
        )
        ...
        if chisq_per_ant is not None:
            bls_for_chisq = [
                ...
                if (
                    (red[0] in sol.vis)
                    and (
                        (split_bl(bl)[0] in sol.gains) | (split_bl(bl)[1] in sol.gains)
                    )
                )
            ]
        ...
        if chisq_per_ant is not None:
            ...
            for ant, cs in cspa.items():
                if ant not in chisq_per_ant:
                    ...
                    chisq_per_ant[ant][~np.isfinite(cs)] = np.zeros_like(
                        cs[~np.isfinite(cs)]
                    )
...
def _init_redcal_meta_dict(nTimes, nFreqs, ants, pol_load_list):
    ...
    redcal_meta["fc_meta"]["polarity_flips"] = {
        ant: np.full(nTimes, np.nan) for ant in ants
    }
    redcal_meta["omni_meta"] = {
        "chisq": {
            str(pols): np.zeros((nTimes, nFreqs), dtype=float) for pols in pol_load_list
        }
    }
    redcal_meta["omni_meta"]["iter"] = {
        str(pols): np.zeros((nTimes, nFreqs), dtype=int) for pols in pol_load_list
    }
    redcal_meta["omni_meta"]["conv_crit"] = {
        str(pols): np.zeros((nTimes, nFreqs), dtype=float) for pols in pol_load_list
    }
    return redcal_meta
...
def _update_redcal_meta(redcal_meta, meta_slice, tSlice, fSlice, pols):
    ...
    for ant in meta_slice["fc_meta"]["dlys"]:
        ...
        redcal_meta["fc_meta"]["polarity_flips"][ant][tSlice] = meta_slice["fc_meta"][
            "polarity_flips"
        ][ant]

    # update omnical metadata
    redcal_meta["omni_meta"]["chisq"][str(pols)][tSlice, fSlice] = meta_slice[
        "omni_meta"
    ]["chisq"]
    redcal_meta["omni_meta"]["iter"][str(pols)][tSlice, fSlice] = meta_slice[
        "omni_meta"
    ]["iter"]
    redcal_meta["omni_meta"]["conv_crit"][str(pols)][tSlice, fSlice] = meta_slice[
        "omni_meta"
    ]["conv_crit"]
...
def count_redundant_nsamples(nsamples, reds, good_ants=None):
    ...
    for red in reds:
        good_bls = [
            bl
            for bl in red
            if (good_ants is None)
            or ((split_bl(bl)[0] in good_ants) and (split_bl(bl)[1] in good_ants))
        ]
...
def redcal_iteration(
    ...
    **filter_reds_kwargs,
    ...
    for pols in pol_load_list:
        ...
        tSlices = [
            slice(start_ind + i, min(start_ind + i + nInt_to_load, stop_ind + 1))
            for i in range(0, nTimes, nInt_to_load)
        ]
        ...
        for tSlice in tSlices:
            ...
            if nInt_to_load == nTimes:  # don't perform partial I/O
                ...
                data, _, nsamples = hd.read(
                    time_range=(hd.times[tSlice][0], hd.times[tSlice][-1]),
                    frequencies=hd.freqs[fSlice],
                    polarizations=pols,
                )

            # run redundant calibration
            meta, sol = redundantly_calibrate(
                data,
                reds,
            ...
            sol.vis = RedDataContainer(
                {
                    red[0]: sol.vis[red[0]]
                    for red in all_reds_this_pol
                    if red[0] in sol.vis
                },
                reds=all_reds_this_pol,
            )

            # make dicts and data containers not in meta or sol. Everything not yet solved for stays flagged.
            first_flags = {
                ant: np.zeros_like(g, dtype=bool) for ant, g in meta["fc_gains"].items()
            }
            omni_flags = {ant: ~np.isfinite(g) for ant, g in sol.gains.items()}
            vissol_flags = RedDataContainer(
                {bl: ~np.isfinite(v) for bl, v in sol.vis.items()}, reds=sol.vis.reds
            )
            vissol_nsamples = count_redundant_nsamples(
                nsamples, all_reds_this_pol, good_ants=sol.gains
            )
            sol.make_sol_finite()
            ...
            hc_first.update(
                gains=meta["fc_gains"], flags=first_flags, tSlice=tSlice, fSlice=fSlice
            )
            total_qual = meta["chisq"]
            if pol_mode in [
                "4pol",
                "4pol_minV",
            ]:  # in which case meta['chisq'] is a numpy array
                total_qual = {ap: meta["chisq"] for ap in hc_omni.pols}
            hc_omni.update(
                gains=sol.gains,
                flags=omni_flags,
                quals=meta["chisq_per_ant"],
...
def redcal_run(
    ...
    **filter_reds_kwargs,
    ...
    while True:
        ...
        redcal_meta, hc_first, hc_omni, hd_vissol = redcal_iteration(
            hd,
        ...
        if run_number == 1 and len(iter0_prefix) > 0:
            ...
            _add_to_history = utils.history_string(
                add_to_history + "\n" + "Iteration 0 Results.\n"
            )
...
def redcal_argparser():
    ...
    redcal_opts.add_argument(
        ...
        default=False,
        action="store_true",
        help="Upsample BDA files to the highest temporal resolution.",
    )
    redcal_opts.add_argument(
        "--downsample",
        default=False,
        action="store_true",
        help="Downsample BDA files to the highest temporal resolution.",
    )
    redcal_opts.add_argument(
        "--pol_mode",
        type=str,
