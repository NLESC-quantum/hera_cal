...
dly_sol_ref, off_sol_ref = info._firstcal_iteration(
    d, df=fqs[1] - fqs[0], f0=fqs[0], medfilt=False, mode="solve"
)
...
if solver == "vqls":
    ...
    info.set_vqls_ansatz(
        RealAmplitudes(num_qubits, entanglement="full", reps=3, insert_barriers=False)
    )
    info.set_vqls_optimizer(opt.COBYLA(maxiter=250))
    info.set_vqls_options({"matrix_decomposition": "pauli", "shots": 10000})

    dly_sol, off_sol = info._firstcal_iteration(
        d, df=fqs[1] - fqs[0], f0=fqs[0], medfilt=False, mode="vqls"
    )
    ...
    dly_sol, off_sol = info._firstcal_iteration(
        d, df=fqs[1] - fqs[0], f0=fqs[0], medfilt=False, mode="vqls_runtime"
    )
    ...
    dly_sol, off_sol = info._firstcal_iteration(
        d, df=fqs[1] - fqs[0], f0=fqs[0], medfilt=False, mode="qubols"
    )
